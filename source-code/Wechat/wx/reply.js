...
exports.reply = function* (next){
	...
	if (message.MsgType === 'event') { //事件推送
		if (message.Event === 'subscribe') {  //订阅事件
			...
			this.body = '您点击了菜单中: '+message.EventKey; //此时的EventKey就是菜单的链接地址
			...
			console.log('您点击了菜单中: '+message.EventKey);
			this.body = message.EventKey; //此时的EventKey就是菜单的链接地址
		...
		if (content === '1') {
			...
			var data = yield wechatApi.uploadMaterial('image',path.join(__dirname,'../1.png'),{type:'image'});
			reply = {
				type:'image',
				mediaId:data.media_id
			}
		}else if(content === '9') {
			var data = yield wechatApi.uploadMaterial('video',path.join(__dirname,'../1.mp4'),{type:'video',description:'{"title":"这是一个标题","introduction":"这是一段描述!"}'});
			reply = {
				type:'video',
				title:'小熊熊玩球',
				description:'小熊熊玩球球，玩球球',
			...
			var qr1 = yield wechatApi.createShorturl(tempQr);
			var qr2 = yield wechatApi.createShorturl(permQr);
			var qr3 = yield wechatApi.createShorturl(permStrQr);
		}else if (content === '19'){
			var longUrl = 'http://www.qzfweb.com';
